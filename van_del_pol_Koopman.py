...
print('Regression residual %f \n' % (
            np.linalg.norm(Y_lift_np - A_lift_np @ X_lift_np - B_lift_np @ U_np, 'fro') / np.linalg.norm(Y_lift_np,
                                                                                                         'fro')))
...
mass_sim = Simulation(ode, delta_t, N, z_init, C, D, E, F, G, F_t, Q, Qf, R, cont_time=True, nonlinear=False,
                      u_0=u_0,
                      xr=zr, ur=ur, collect=False, est=False, mu=mu, sigma=sigma, beta=1,
                      sin_const=sin_const, N_sample=1, epsilon=0.1, N_sim=200, data_set=None, N_sample_max=None, x_0=x0)
